# win_firewall_public_logging_dropped_packets

This role remediates CIS Benchmark 9.3.8 (check 26137): Ensure 'Windows Firewall: Public: Logging: Log dropped packets' is set to 'Yes'.
...
|----------|---------|-------------|
| `win_firewall_public_logging_dropped_packets_log_dropped_packets` | `1` | Registry value for LogDroppedPackets in Public Profile Logging. 0 = No, 1 = Yes (CIS requirement). |
...
```yaml
---
- name: Remediate Windows Firewall Public Profile Logging Dropped Packets
  hosts: windows_servers
  gather_facts: yes
  roles:
    - twanus.wazuh_fixer.win_firewall_public_logging_dropped_packets
```
...
```yaml
---
- name: Test Windows Firewall Public Profile Logging Dropped Packets Role (Development)
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: ../roles/win_firewall_public_logging_dropped_packets  # noqa role-name[path]
  post_tasks:
...
2. Navigate to: `Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Public Profile\Logging Customize\Log dropped packets`
3. Verify that "Log dropped packets" is set to "Yes"

Or use PowerShell:

```powershell
Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile\Logging" -Name "LogDroppedPackets" | Select-Object -ExpandProperty LogDroppedPackets
```
...
- **CIS Benchmark**: 9.3.8 (check 26137)
- **CIS CSC v7**: 6.3, 9.4
- **CIS CSC v8**: 4.5, 8.5
...
The Wazuh check (26137) passes if all of the following conditions are met (Condition: all):
...
- This setting applies specifically to the Public Profile logging, which is used when the device is connected to a public network (untrusted network)
- Logged events will appear in the firewall log file with "DROP" in the action column
- The firewall logging file location is configured separately (typically `%SystemRoot%\System32\logfiles\firewall\publicfw.log`)
