# win_firewall_public_logging_successful_connections

This role remediates CIS Benchmark 9.3.9 (check 26138): Ensure 'Windows Firewall: Public: Logging: Log successful connections' is set to 'Yes'.
...
|----------|---------|-------------|
| `win_firewall_public_logging_successful_connections_value` | `1` | Registry value for LogSuccessfulConnections in Public Profile Logging. 0 = No, 1 = Yes (CIS requirement). |
...
```yaml
---
- name: Remediate Windows Firewall Public Profile Logging Successful Connections
  hosts: windows_servers
  gather_facts: yes
  roles:
    - twanus.wazuh_fixer.win_firewall_public_logging_successful_connections
```
...
```yaml
---
- name: Test Windows Firewall Public Profile Logging Successful Connections Role (Development)
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: ../roles/win_firewall_public_logging_successful_connections  # noqa role-name[path]
  post_tasks:
...
2. Navigate to: `Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Public Profile\Logging Customize\Log successful connections`
3. Verify that "Log successful connections" is set to "Yes"

Or use PowerShell:

```powershell
Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile\Logging" -Name "LogSuccessfulConnections" | Select-Object -ExpandProperty LogSuccessfulConnections
```
...
- **CIS Benchmark**: 9.3.9 (check 26138)
- **CIS CSC v7**: 6.3, 9.4
- **CIS CSC v8**: 4.5, 8.5
...
The Wazuh check (26138) passes if all of the following conditions are met:
...
- This setting applies specifically to the Public Profile, which is used when the device is connected to a public network (such as airports, coffee shops, etc.) where it should not trust other devices on the network
