# win_iis_admin_service_disabled

This role remediates CIS Benchmark 5.6 (check 26077): Ensure 'IIS Admin Service (IISADMIN)' is set to 'Disabled' or 'Not Installed'.
...
When Disabled, the IIS Admin Service is disabled and will not start, preventing IIS administration functionality and reducing the attack surface.
...
|----------|---------|-------------|
| `win_iis_admin_service_disabled_start_value` | `4` | Service Start registry value (4 = Disabled) |
...
```yaml
---
- name: Disable IIS Admin Service
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: twanus.wazuh_fixer.win_iis_admin_service_disabled
```
...
- **Registry Path**: `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\IISADMIN`
- **Value Name**: `Start`
...
Test-Path -Path "HKLM:\SYSTEM\CurrentControlSet\Services\IISADMIN"

# If it exists, check the Start value
Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\IISADMIN" -Name "Start" -ErrorAction SilentlyContinue | Select-Object Start
```

Or using the registry editor:

1. Open `regedit.exe`
2. Navigate to `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\IISADMIN`
...
1. Open `gpedit.msc`
2. Navigate to `Computer Configuration\Policies\Windows Settings\Security Settings\System Services\IIS Admin Service`
...
- **Security benefit**: Disabling the IIS Admin Service reduces the attack surface by preventing IIS administration functionality. Hosting a website from a workstation is an increased security risk, as the attack surface of that workstation is then greatly increased.

- **Default installation**: This service is not installed by default. It is supplied with Windows, but is installed by enabling an optional Windows feature (Internet Information Services).
...
- **Group Policy alternative**: This setting can also be configured via Group Policy at: `Computer Configuration\Policies\Windows Settings\Security Settings\System Services\IIS Admin Service`
