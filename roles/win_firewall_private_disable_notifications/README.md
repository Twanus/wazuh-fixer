# win_firewall_private_disable_notifications

This role remediates CIS Benchmark 9.2.3 (check 26125): Ensure 'Windows Firewall: Private: Settings: Display a notification' is set to 'No'.

## Description

Windows Firewall with Advanced Security can display notifications to users when a program is blocked from receiving inbound connections. The Private Profile applies to networks at locations where the user trusts the other computers and devices on the network, such as a home or office network.
...
|----------|---------|-------------|
| `win_firewall_private_disable_notifications_disable_notifications` | `1` | Registry value for DisableNotifications in Private Profile. 0 = Yes (display notifications), 1 = No (disable notifications, CIS requirement). |
...
```yaml
---
- name: Remediate Windows Firewall Private Profile Display Notification Setting
  hosts: windows_servers
  gather_facts: yes
  roles:
    - twanus.wazuh_fixer.win_firewall_private_disable_notifications
```
...
```yaml
---
- name: Test Windows Firewall Private Profile Display Notification Role (Development)
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: ../roles/win_firewall_private_disable_notifications  # noqa role-name[path]
  post_tasks:
...
2. Navigate to: `Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Private Profile\Settings Customize\Display a notification`
3. Verify that "Display a notification" is set to "No"

Or use PowerShell:

```powershell
Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile" -Name "DisableNotifications" | Select-Object -ExpandProperty DisableNotifications
```
...
- **CIS Benchmark**: 9.2.3 (check 26125)
- **CIS CSC v7**: 9.4, 11.2
- **CIS CSC v8**: 4.5
...
`Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Private Profile\Settings Customize\Display a notification`

## Checks

The Wazuh check (26125) passes if all of the following conditions are met:

- The registry key `HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile` exists, AND
...
- This setting applies specifically to the Private Profile, which is used when the device is connected to a trusted network (such as home or office networks)
