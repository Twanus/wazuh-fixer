# win_firewall_public_disable_notifications

This role remediates CIS Benchmark 9.3.3 (check 26132): Ensure 'Windows Firewall: Public: Settings: Display a notification' is set to 'No'.

## Description

Windows Firewall with Advanced Security can display notifications to users when a program is blocked from receiving inbound connections. The Public Profile applies to networks where the host is connected to a public network, such as those found in airports, coffee shops, and other public locations.
...
|----------|---------|-------------|
| `win_firewall_public_disable_notifications_disable_notifications` | `1` | Registry value for DisableNotifications in Public Profile. 0 = Yes (display notifications), 1 = No (disable notifications, CIS requirement). |
...
```yaml
---
- name: Remediate Windows Firewall Public Profile Display Notification Setting
  hosts: windows_servers
  gather_facts: yes
  roles:
    - twanus.wazuh_fixer.win_firewall_public_disable_notifications
```
...
```yaml
---
- name: Test Windows Firewall Public Profile Display Notification Role (Development)
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: ../roles/win_firewall_public_disable_notifications  # noqa role-name[path]
  post_tasks:
...
2. Navigate to: `Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Public Profile\Settings Customize\Display a notification`
3. Verify that "Display a notification" is set to "No"

Or use PowerShell:

```powershell
Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile" -Name "DisableNotifications" | Select-Object -ExpandProperty DisableNotifications
```
...
- **CIS Benchmark**: 9.3.3 (check 26132)
- **CIS CSC v7**: 9.4, 11.2
- **CIS CSC v8**: 4.5
...
Some organizations may prefer to avoid alarming users when firewall rules block certain types of network activity. However, notifications can be helpful when troubleshooting network issues involving the firewall. Disabling notifications prevents user confusion and reduces the risk of users making incorrect decisions when prompted about firewall blocks, especially on public networks where security is critical.
...
The Wazuh check (26132) passes if all of the following conditions are met:
...
- **Public Network Security**: Particularly important for public networks where security is critical

## Notes

- This setting applies specifically to the Public Profile, which is used when the device is connected to a public network
