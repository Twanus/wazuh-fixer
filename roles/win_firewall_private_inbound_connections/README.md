# win_firewall_private_inbound_connections

This role remediates CIS Benchmark 9.2.2 (check 26124): Ensure 'Windows Firewall: Private: Inbound connections' is set to 'Block (default)'.

## Description

Windows Firewall with Advanced Security provides host-based, two-way network traffic filtering for a device and blocks unauthorized network traffic flowing into or out of the local device. The Private Profile applies to networks where the host is connected to a private network.
...
|----------|---------|-------------|
| `win_firewall_private_inbound_connections_default_inbound_action` | `1` | Registry value for DefaultInboundAction in Private Profile. 0 = Allow, 1 = Block (default) (CIS requirement). |
...
```yaml
---
- name: Remediate Windows Firewall Private Profile Inbound Connections
  hosts: windows_servers
  gather_facts: yes
  roles:
    - twanus.wazuh_fixer.win_firewall_private_inbound_connections
```
...
```yaml
---
- name: Test Windows Firewall Private Profile Inbound Connections Role (Development)
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: ../roles/win_firewall_private_inbound_connections  # noqa role-name[path]
  post_tasks:
...
2. Navigate to: `Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Private Profile\Inbound connections`
3. Verify that "Inbound connections" is set to "Block (default)"

Or use PowerShell:

```powershell
Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile" -Name "DefaultInboundAction" | Select-Object -ExpandProperty DefaultInboundAction
```
...
- **CIS Benchmark**: 9.2.2 (check 26124)
- **CIS CSC v7**: 9.4, 11.2
- **CIS CSC v8**: 4.5
...
The Wazuh check (26124) passes if any of the following conditions are met:
