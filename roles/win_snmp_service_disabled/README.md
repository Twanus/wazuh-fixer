# win_snmp_service_disabled

This role remediates CIS Benchmark 5.28 (check 26099): Ensure 'SNMP Service (SNMP)' is set to 'Disabled' or 'Not Installed'.
...
When Disabled, the SNMP Service is disabled and will not start, preventing SNMP functionality and reducing the attack surface.
...
|----------|---------|-------------|
| `win_snmp_service_disabled_start_value` | `4` | Service Start registry value (4 = Disabled) |
...
- name: Disable SNMP Service
  ...
  become: false
  roles:
    - role: twanus.wazuh_fixer.win_snmp_service_disabled
```
...
- **CIS Benchmark**: 5.28 (check 26099)
- **CIS CSC v7**: 9.2
- **CIS CSC v8**: 4.8
...
- **Registry Path**: `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\SNMP`
- **Value Name**: `Start`
...
Test-Path -Path "HKLM:\SYSTEM\CurrentControlSet\Services\SNMP"

# If it exists, check the Start value
Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\SNMP" -Name "Start" -ErrorAction SilentlyContinue | Select-Object Start
```
...
1. Open `gpedit.msc`
2. Navigate to `Computer Configuration\Policies\Windows Settings\Security Settings\System Services\SNMP Service`
...
- **Security benefit**: Disabling the SNMP Service reduces the attack surface by preventing SNMP functionality. Features that enable inbound network connections increase the attack surface. In a high security environment, management of secure workstations should be handled locally.

- **Default installation**: This service is not installed by default. It is supplied with Windows, but is installed by enabling an optional Windows feature (Simple Network Management Protocol (SNMP)).
...
- **Group Policy alternative**: This setting can also be configured via Group Policy at: `Computer Configuration\Policies\Windows Settings\Security Settings\System Services\SNMP Service`
