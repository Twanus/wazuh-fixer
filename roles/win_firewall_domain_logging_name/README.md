# win_firewall_domain_logging_name

This role remediates CIS Benchmark 9.1.4 (check 26119): Ensure 'Windows Firewall: Domain: Logging: Name' is set to '%SystemRoot%\System32\logfiles\firewall\domainfw.log'.
...
|----------|---------|-------------|
| `win_firewall_domain_logging_name_log_file_path` | `%SystemRoot%\System32\logfiles\firewall\domainfw.log` | Registry value for LogFilePath in Domain Profile Logging. The recommended path for the Domain Profile firewall log file (CIS requirement). |
...
```yaml
---
- name: Remediate Windows Firewall Domain Profile Logging Name
  hosts: windows_servers
  gather_facts: yes
  roles:
    - twanus.wazuh_fixer.win_firewall_domain_logging_name
```
...
```yaml
---
- name: Test Windows Firewall Domain Profile Logging Name Role (Development)
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: ../roles/win_firewall_domain_logging_name  # noqa role-name[path]
  post_tasks:
...
- **Recommended Value**: `%SystemRoot%\System32\logfiles\firewall\domainfw.log`
- **Purpose**: Specifies the path and name of the file in which Windows Firewall will write its log information for the Domain Profile
...
2. Navigate to: `Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Domain Profile\Logging Customize\Name`
3. Verify that "Name" is set to `%SystemRoot%\System32\logfiles\firewall\domainfw.log`

Or use PowerShell:

```powershell
Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile\Logging" -Name "LogFilePath" -ErrorAction SilentlyContinue | Select-Object -ExpandProperty LogFilePath
```

Expected output: `%SystemRoot%\System32\logfiles\firewall\domainfw.log`

## Compliance

This role addresses the following compliance frameworks:

- **CIS Benchmark**: 9.1.4 (check 26119)
- **CIS CSC v7**: 6.3, 9.4
- **CIS CSC v8**: 8.2
...
To establish the recommended configuration via Group Policy, set the following UI path to %SystemRoot%\System32\logfiles\firewall\domainfw.log:

`Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Domain Profile\Logging Customize\Name`

## Checks

The Wazuh check (26119) passes if all of the following conditions are met (Condition: all):
...
- The registry value `LogFilePath` equals `%SystemRoot%\System32\logfiles\firewall\domainfw.log`
...
- **Audit Trail**: Provides a dedicated log file for Domain Profile firewall events
...
- This setting applies specifically to the Domain Profile logging configuration
...
- This role only configures the Domain Profile logging; other profiles (Private, Public) may need separate configuration
