# win_firewall_public_inbound_connections

This role remediates CIS Benchmark 9.3.2 (check 26131): Ensure 'Windows Firewall: Public: Inbound connections' is set to 'Block (default)'.
...
|----------|---------|-------------|
| `win_firewall_public_inbound_connections_default_inbound_action` | `1` | Registry value for DefaultInboundAction in Public Profile. 0 = Allow, 1 = Block (default) (CIS requirement). |
...
```yaml
---
- name: Remediate Windows Firewall Public Profile Inbound Connections
  hosts: windows_servers
  gather_facts: yes
  roles:
    - twanus.wazuh_fixer.win_firewall_public_inbound_connections
```
...
```yaml
---
- name: Test Windows Firewall Public Profile Inbound Connections Role (Development)
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: ../roles/win_firewall_public_inbound_connections  # noqa role-name[path]
  post_tasks:
...
2. Navigate to: `Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Public Profile\Inbound connections`
3. Verify that "Inbound connections" is set to "Block (default)"

Or use PowerShell:

```powershell
Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile" -Name "DefaultInboundAction" | Select-Object -ExpandProperty DefaultInboundAction
```
...
- **CIS Benchmark**: 9.3.2 (check 26131)
- **CIS CSC v7**: 9.4, 11.2
- **CIS CSC v8**: 4.5
...
The Wazuh check (26131) passes if any of the following conditions are met:
...
- **Public Network Security**: Provides critical protection when devices are connected to untrusted public networks
...
- The Public Profile is typically the most restrictive profile, as public networks are considered untrusted
