# win_firewall_public_logging_name

This role remediates CIS Benchmark 9.3.6 (check 26135): Ensure 'Windows Firewall: Public: Logging: Name' is set to '%SystemRoot%\System32\logfiles\firewall\publicfw.log'.

## Description

Windows Firewall with Advanced Security provides host-based, two-way network traffic filtering for a device and blocks unauthorized network traffic flowing into or out of the local device. The Public Profile applies to networks that are not trusted, such as public Wi-Fi networks.
...
|----------|---------|-------------|
| `win_firewall_public_logging_name_log_file_path` | `%SystemRoot%\System32\logfiles\firewall\publicfw.log` | Registry value for LogFilePath in Public Profile Logging. The recommended path for the Public Profile firewall log file (CIS requirement). |
...
```yaml
---
- name: Remediate Windows Firewall Public Profile Logging Name
  hosts: windows_servers
  gather_facts: yes
  roles:
    - twanus.wazuh_fixer.win_firewall_public_logging_name
```
...
```yaml
---
- name: Test Windows Firewall Public Profile Logging Name Role (Development)
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: ../roles/win_firewall_public_logging_name  # noqa role-name[path]
  post_tasks:
...
- **Recommended Value**: `%SystemRoot%\System32\logfiles\firewall\publicfw.log`
- **Purpose**: Specifies the path and name of the file in which Windows Firewall will write its log information for the Public Profile
...
2. Navigate to: `Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Public Profile\Logging Customize\Name`
3. Verify that "Name" is set to `%SystemRoot%\System32\logfiles\firewall\publicfw.log`

Or use PowerShell:

```powershell
Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\PublicProfile\Logging" -Name "LogFilePath" -ErrorAction SilentlyContinue | Select-Object -ExpandProperty LogFilePath
```

Expected output: `%SystemRoot%\System32\logfiles\firewall\publicfw.log`

## Compliance

This role addresses the following compliance frameworks:

- **CIS Benchmark**: 9.3.6 (check 26135)
- **CIS CSC v7**: 6.3, 9.4
- **CIS CSC v8**: 8.2
...
To establish the recommended configuration via Group Policy, set the following UI path to %SystemRoot%\System32\logfiles\firewall\publicfw.log:
...
The Wazuh check (26135) passes if all of the following conditions are met (Condition: all):
...
- The registry value `LogFilePath` equals `%SystemRoot%\System32\logfiles\firewall\publicfw.log`
...
- **Audit Trail**: Provides a dedicated log file for Public Profile firewall events
...
- This setting applies specifically to the Public Profile logging configuration
