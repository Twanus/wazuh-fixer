# win_firewall_domain_logging_size_limit

This role remediates CIS Benchmark 9.1.5 (check 26120): Ensure 'Windows Firewall: Domain: Logging: Size limit (KB)' is set to '16,384 KB or greater'.
...
The logging size limit setting specifies the maximum size of the file in which Windows Firewall will write its log information for the Domain Profile. When the log file reaches this size limit, Windows Firewall will stop logging new events unless logging is configured to overwrite old entries.
...
|----------|---------|-------------|
| `win_firewall_domain_logging_size_limit_log_file_size` | `16384` | Registry value for LogFileSize in Domain Profile Logging (in KB). Must be >= 16384 KB per CIS requirement. |
...
```yaml
---
- name: Remediate Windows Firewall Domain Profile Logging Size Limit
  hosts: windows_servers
  gather_facts: yes
  roles:
    - twanus.wazuh_fixer.win_firewall_domain_logging_size_limit
```
...
```yaml
---
- name: Test Windows Firewall Domain Profile Logging Size Limit Role (Development)
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: ../roles/win_firewall_domain_logging_size_limit  # noqa role-name[path]
  post_tasks:
...
2. Navigate to: `Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Domain Profile\Logging Customize\Size limit (KB)`
3. Verify that "Size limit (KB)" is set to "16,384 KB or greater"

Or use PowerShell:

```powershell
Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile\Logging" -Name "LogFileSize" | Select-Object -ExpandProperty LogFileSize
```
...
- **CIS Benchmark**: 9.1.5 (check 26120)
- **CIS CSC v7**: 6.4, 9.4
- **CIS CSC v8**: 8.3
...
`Computer Configuration\Policies\Windows Settings\Security Settings\Windows Defender Firewall with Advanced Security\Windows Defender Firewall with Advanced Security\Windows Firewall Properties\Domain Profile\Logging Customize\Size limit (KB)`

## Checks

The Wazuh check (26120) passes if all of the following conditions are met:
...
- This setting applies specifically to the Domain Profile logging, which is used when the device is connected to a network where it can authenticate to a domain controller
