# win_peer_networking_identity_manager_disabled

This role remediates CIS Benchmark 5.15 (check 26086): Ensure 'Peer Networking Identity Manager (p2pimsvc)' is set to 'Disabled'.
...
When Disabled, the Peer Networking Identity Manager service is disabled and will not start, preventing peer-to-peer identity services and reducing the attack surface.
...
|----------|---------|-------------|
| `win_peer_networking_identity_manager_disabled_start_value` | `4` | Service Start registry value (4 = Disabled) |
...
```yaml
---
- name: Disable Peer Networking Identity Manager Service
  hosts: windows
  gather_facts: true
  become: false
  roles:
    - role: twanus.wazuh_fixer.win_peer_networking_identity_manager_disabled
```
...
- **CIS Benchmark**: 5.15 (check 26086)
- **CIS CSC v7**: 9.2
- **CIS CSC v8**: 4.8
...
- **Registry Path**: `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\p2pimsvc`
- **Value Name**: `Start`
...
Test-Path -Path "HKLM:\SYSTEM\CurrentControlSet\Services\p2pimsvc"

# If it exists, check the Start value
Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\p2pimsvc" -Name "Start" -ErrorAction SilentlyContinue | Select-Object Start
```

Or using the registry editor:

1. Open `regedit.exe`
2. Navigate to `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\p2pimsvc`
...
1. Open `gpedit.msc`
2. Navigate to `Computer Configuration\Policies\Windows Settings\Security Settings\System Services\Peer Networking Identity Manager`
...
- **Security benefit**: Disabling the Peer Networking Identity Manager service reduces the attack surface by preventing peer-to-peer identity services. In a high security environment, it is more secure to rely on centralized name resolution methods maintained by authorized staff rather than distributed peer-to-peer name resolution.
...
- **Group Policy alternative**: This setting can also be configured via Group Policy at: `Computer Configuration\Policies\Windows Settings\Security Settings\System Services\Peer Networking Identity Manager`
